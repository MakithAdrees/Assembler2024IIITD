    ...
    for a in range(len(lines)):
        ...
        if lines[a][0][-1] != ':':
            ...
            if lines[a][1].count(',') == 2:
                reg = lines[a][1].split(',')
                try:
                    output.append(ff(instruction, reg[0], reg[1], int(reg[2]) , a+1))
                except:   
                    if reg[2] in register:
                        output.append(ff(instruction, reg[0], reg[1], reg[2] , a+1))
                    elif reg[2] in labels:
                        minn = LabelToImm(labels, a, reg[2])
                        output.append(ff(instruction, reg[0], reg[1], 4*(a-minn) , a+1))
                    else:
                        output.append(ff(instruction, reg[0], reg[1], 'invalid' , a+1))
                ...
                reg2 = reg[1].split('(')
                try:
                    output.append(ff(instruction, reg[0], reg2[1][:-1], int(reg2[0]) , a+1))
                except:
                    output.append(ff(instruction, reg[0], reg2[1][:-1], 'InvalidImmediateVal' , a+1))
            
            elif lines[a][1].count(',') == 1:
                reg = lines[a][1].split(',')
                try:
                    output.append(ff(instruction, reg[0], int(reg[1]) , a+1))
                except:    
                    if reg[1] in register:
                        output.append(ff(instruction, reg[0], reg[1] , a+1))
                    elif reg[1] in labels:
                        minn = LabelToImm(labels, a, reg[1])
                        
                        output.append(ff(instruction, reg[0], 4*(a-minn) , a+1))
